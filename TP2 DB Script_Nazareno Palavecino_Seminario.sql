...
CREATE TABLE IF NOT EXISTS `Torneo_JiuJitsu_DB`.`llave` (
    ...
    `categoría_edad` VARCHAR(20) NOT NULL,
    `dni_ganador` VARCHAR(20) NOT NULL,
	PRIMARY KEY (`id_llave`),
    FOREIGN KEY (`dni_ganador`) REFERENCES `Torneo_JiuJitsu_DB`.`competidor` (`dni`));
...
CREATE TABLE IF NOT EXISTS `Torneo_JiuJitsu_DB`.`competidor_encuentro` (
    `id_competidor_encuentro` INT NOT NULL AUTO_INCREMENT,
...
`categoría`, `cinturón`, `escuela`) 
VALUES 
	('41111111', 'Juan', 'Juarez', '19', 'masculino', 'medio', 'blanco', 'Gracie'),
    ('42222222', 'Pedro', 'Pomez', '20', 'masculino', 'medio', 'blanco', 'Kimura'),
    ('43333333', 'Marcos', 'Marquez', '21', 'masculino', 'medio', 'blanco', 'Valdez'),
	('44444444', 'Gastón', 'Gutierrez', '22', 'masculino', 'medio', 'blanco', 'Ibarra');

INSERT INTO `torneo_jiujitsu_db`.`llave` (`id_llave`, `categoría_sexo`, `categoría_peso`, 
`categoría_cinturón`) 
VALUES ('01', 'masculino', 'medio', 'blanco');
